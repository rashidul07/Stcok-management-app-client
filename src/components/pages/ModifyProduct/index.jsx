...
const ModifyProduct = () => {
    ...
    useEffect(() => {
        if (productType && productType.value !== '') {
            ...
            if (!storageModifiedData) {
                localStorage.setItem(`modified${productType.value}List`, JSON.stringify([]));
            }
        }
    }, [productType?.value])
    ...
    return (
        <div className="p-12 md:m-auto md:border-2 max-sm:w-screen">
            ...
            <RemoveForm localProducts={localProducts} setLocalProducts={setLocalProducts} productType={productType} setProductType={setProductType} modifiedProductList={modifiedProductList} setModifiedProductList={setModifiedProductList} />
